...
cmd=$(dpkg -s chrony | grep -E '(Status:|not installed)' 2> \dev\null)
cmd1=$(systemctl is-enabled systemd-timesyncd 2> \dev\null)
cmd2=$(grep "^restrict" /etc/ntp.conf 2> \dev\null)
cmd3=$(grep -E "^(server|pool)" /etc/ntp.conf 2> \dev\null)
cmd4=$(grep "RUNASUSER=ntp" /etc/init.d/ntp 2> \dev\null)
correct=""
correct1="masked"
